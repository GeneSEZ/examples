 ...
 */
public class Line extends AbstractForm implements Form {
	...
	public void rotate(Point center, double phi) {
		...
		start.rotate(center, phi);
		end.rotate(center, phi);
		/* PROTECTED REGION END */
	}
	...
	public void repaint(Color color, boolean solid) {
		...
		if (theShape != null) {
			Scene.getBoard().removeShape(start);
		}
		theShape = Scene.getBoard().drawLine(start.getX(), start.getY(), end.getX(), end.getY());
		/* PROTECTED REGION END */
	}
